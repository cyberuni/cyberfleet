...
Guide for developing `cyberfleet` locally. AI coding assistants should also read [AGENTS.md](AGENTS.md).
...
pnpm cf dev --help       # run the CLI from source, no build (tsx)
pnpm cf test:watch       # watch mode
```
...
manifests under `packages/cyberfleet/` — add any new manifest to that script's list.

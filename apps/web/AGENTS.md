...
Astro + Starlight docs for cyberfleet, deployed to GitHub Pages. The site is served from a
`base` path (`/cyberfleet`), so local dev URLs are `http://localhost:4321/cyberfleet/...`, not
the bare root.
...
7. Build and crawl for broken internal links:
   ```sh
   pnpm web build
   cd apps/web/dist && for f in $(find . -name '*.html'); do grep -o 'href="/cyberfleet/[a-z0-9/-]*"' $f; done \
     | sort -u | sed 's|href="/cyberfleet/||;s|"||' \
